...
void SoftmaxWithViewLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    Dtype weight = 0;
    const Dtype* label = bottom[1]->cpu_data();
    int num = prob_.num();  //the amount of batch
    int dim = prob_.count() / num;  //class of fc layer if 12 classes, then is 4320
    int spatial_dim = prob_.height() * prob_.width();   //if it is fc layer, then height = width = 1
    for (int i = 0; i < num; ++i) {
      for (int j = 0; j < spatial_dim; ++j) {
         int label_value = static_cast<int>(label[i * spatial_dim + j]);
